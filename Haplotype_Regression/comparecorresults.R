...
cl <- bind_rows(df_full1 %>% select(pearson_cor,traitpair,selow,sehigh,pearson_p) %>% rename(cor=pearson_cor,p=pearson_p) %>% mutate(measure="hapregb1"),
                df_full2 %>% select(pearson_cor,traitpair,selow,sehigh,pearson_p) %>% rename(cor=pearson_cor,p=pearson_p) %>% mutate(measure="hapregb2"),
                df_full3 %>% select(pearson_cor,traitpair,selow,sehigh,pearson_p) %>% rename(cor=pearson_cor,p=pearson_p) %>% mutate(measure="hapregb3"),
                gct %>% select(rg,traitpair,selow,sehigh,p) %>% rename(cor=rg) %>% mutate(measure="gc"),
                pfilt %>% select(phenocor,traitpair,selow,sehigh,p) %>% rename(cor=phenocor) %>% mutate(measure="pc"))
cl$cor <- as.numeric(ifelse(cl$cor>1,1,ifelse(cl$cor<(-1),-1,cl$cor)))

# Check cor between IBD and IBD PSC
filter(cl,traitpair=="K11_IBD_STRICT-K11_IBD_STRICT_PSC") %>% mutate(se=sehigh-cor)
...
ggsave(paste0(fdir,"snps",nsnp,"/figures_askout07012024/fig6a_corr.tiff"),height=5,width=10)
...
ggsave(paste0(fdir,"snps",nsnp,"/figures_askout07012024/fig6b_corrpair.tiff"),height=7,width=12)
...
ggplot(cl %>% filter(traitpair=="K11_IBD_STRICT-K11_IBD_STRICT_PSC"),
       aes(x=cor,y=measure,fill=measure))+
  geom_bar(stat="identity")+
  geom_errorbar(aes(xmin=selow,xmax=sehigh),width=0.2)+ # error bars are cor +- SE
  labs(x="Correlation Value",y="Correlation Measure",title="K11_IBD_STRICT-K11_IBD_STRICT_PSC")+
  guides(fill="none")+
  scale_y_discrete(labels=c("Genetic Correlation","Hap Reg Block 1","Hap Reg Block 2","Hap Reg Block 3","Phenotypic Correlation"))+
  theme_classic()
ggsave(paste0(fdir,"snps",nsnp,"/figures_askout07012024/ibd_ibdpsccorr.tiff"),height=7,width=7)

######################## hapclusterandregress_check2.R
## Written by Courtney Smith 9-8-2024
## Goal of script: Defining clusters and running haplotype regression pipeline - Blocks 1-3
# Repeat this with a second set of randomly selected 1000 snps
...
for (nsnp in nsnps){
  for (bnum in blocks){
    hla_dir="/home/ivm/haplo_analysis/check2/"
...
##cd /home/ivm/haplo_analysis/check2/snps1000
...
for (nsnp in nsnps){
  for (bnum in blocks){
    ...
    col_vector = c("dodgerblue2", "#E31A1C", "green4","#6A3D9A", "#FF7F00","black", "gold1", "skyblue2", "#FB9A99", "palegreen2",  "#CAB2D6", "#FDBF6F", "gray70", "khaki2",  "maroon", "orchid1", "deeppink1", "blue1", "steelblue4",  "darkturquoise", "green1", "yellow4", "yellow3","darkorange4", "brown","purple")
...
for (nsnp in nsnps){
  for (bnum in blocks){
    hla_dir="/home/ivm/haplo_analysis/check3/"
    nsnp_dir=paste0(hla_dir,"snps",nsnp,"/")
    setwd(nsnp_dir)

    # Load in snps in the block, randomly subset
    hb_all<-fread(paste0("/home/ivm/from_satu/hbg_full_sets/hbg",bnum,"_maf05.txt"), header = F) # all snps in the block w/ MAF > 5%; generated at top of hapclusterandregress_check5.R
    hb_all_snp <- hb_all[grep("^chr6_\\d+_[A-Z]_[A-Z]$", hb_all$V1), , drop = FALSE] # make sure just single nucleotide to single nucleotide
    set.seed(897)
    hb<-hb_all_snp %>% sample_n(nsnp)
    #hb<-hb_all %>% sample_n(nsnp)
...
####### Create the genotype file for the snps in this block with the alleles (A,T,C,Gs)
##extract the snps with PLINK
##cd /home/ivm/haplo_analysis/check3/snps1000
##plink --bfile /finngen/library-red/finngen_R10/genotype_plink_1.0/data/finngen_R10 --keep-allele-order --keep /home/ivm/from_satu/general_files/individuals2.txt --recode --extract hb3_snps.txt --out hb3_genotypes
##plink --bfile /finngen/library-red/finngen_R10/genotype_plink_1.0/data/finngen_R10 --keep-allele-order --keep /home/ivm/from_satu/general_files/individuals2.txt --recode --extract hb2_snps.txt --out hb2_genotypes
##plink --bfile /finngen/library-red/finngen_R10/genotype_plink_1.0/data/finngen_R10 --keep-allele-order --keep /home/ivm/from_satu/general_files/individuals2.txt --recode --extract hb1_snps.txt --out hb1_genotypes
...
for (nsnp in nsnps){
  for (bnum in blocks){
    ...
    #qual_col_pals = brewer.pal.info[brewer.pal.info$category == 'qual',]
    #col_vector = unlist(mapply(brewer.pal, qual_col_pals$maxcolors, rownames(qual_col_pals)))
    col_vector = c("dodgerblue2", "#E31A1C", "green4","#6A3D9A", "#FF7F00","black", "gold1", "skyblue2", "#FB9A99", "palegreen2",  "#CAB2D6", "#FDBF6F", "gray70", "khaki2",  "maroon", "orchid1", "deeppink1", "blue1", "steelblue4",  "darkturquoise", "green1", "yellow4", "yellow3","darkorange4", "brown","purple","white","aquamarine","yellowgreen","aliceblue","bisque1","azure4","darkseagreen")
    colors <- data.frame(final_cluster=unique(allplot$final_cluster),
                         color= sample(col_vector,size=nplot))
...
######################## hapclusterandregress_check4.R
## Written by Courtney Smith 9-8-2024
## Goal of script: Defining clusters and running haplotype regression pipeline - Blocks 1-3 - 30,100,150 SNPs
# Repeat this with a second set of randomly selected 1000 snps with MAF 1 to 5%
...
for (nsnp in nsnps){
  for (bnum in blocks){
    hla_dir="/home/ivm/haplo_analysis/check4/"
    nsnp_dir=paste0(hla_dir,"snps",nsnp,"/")
    setwd(nsnp_dir)

    # Load in snps in the block, randomly subset
    hb_all<-fread(paste0("/home/ivm/from_satu/hbg_full_sets/hbg",bnum,"_maf01to05.txt"), header = F) # all snps in the block w/ MAF 1 to 5%; generated at top of hapclusterandregress_check5.R
    hb_all_snp <- hb_all[grep("^chr6_\\d+_[A-Z]_[A-Z]$", hb_all$V1), , drop = FALSE] # make sure just single nucleotide to single nucleotide
    set.seed(721)
    hb<-hb_all_snp %>% sample_n(nsnp)
    #hb<-hb_all %>% sample_n(nsnp)
...
####### Create the genotype file for the snps in this block with the alleles (A,T,C,Gs)
##extract the snps with PLINK
##cd /home/ivm/haplo_analysis/check4/snps1000
##plink --bfile /finngen/library-red/finngen_R10/genotype_plink_1.0/data/finngen_R10 --keep-allele-order --keep /home/ivm/from_satu/general_files/individuals2.txt --recode --extract hb3_snps.txt --out hb3_genotypes
##plink --bfile /finngen/library-red/finngen_R10/genotype_plink_1.0/data/finngen_R10 --keep-allele-order --keep /home/ivm/from_satu/general_files/individuals2.txt --recode --extract hb2_snps.txt --out hb2_genotypes
##plink --bfile /finngen/library-red/finngen_R10/genotype_plink_1.0/data/finngen_R10 --keep-allele-order --keep /home/ivm/from_satu/general_files/individuals2.txt --recode --extract hb1_snps.txt --out hb1_genotypes
...
for (nsnp in nsnps){
  for (bnum in blocks){
    ...
    col_vector = c("dodgerblue2", "#E31A1C", "green4","#6A3D9A", "#FF7F00","black", "gold1", "skyblue2", "#FB9A99", "palegreen2",  "#CAB2D6", "#FDBF6F", "gray70", "khaki2",  "maroon", "orchid1", "deeppink1", "blue1", "steelblue4",  "darkturquoise", "green1", "yellow4", "yellow3","darkorange4", "brown","purple","white","aquamarine","yellowgreen","aliceblue","bisque1","azure4","darkseagreen")
    colors <- data.frame(final_cluster=unique(allplot$final_cluster),
                         color= sample(col_vector,size=nplot,replace=TRUE))
...
######################## hapclusterandregress_check5.R
## Written by Courtney Smith 9-20-2024
## Goal of script: Defining clusters and running haplotype regression pipeline - Blocks 1-3
# Repeat this with another set of randomly selected 1000 snps with 500 of MAF > 5% and 500 of MAF 1 to 5%
...
########### STEP 1: Reformat SNP files (only need to do this once per snp file)
library(dplyr)

an <- data.table::fread("/home/ivm/from_satu/general_files/rsids.txt.gz")

for (bnum in blocks) {
  snps <- data.table::fread(paste0("/finngen/green/hbg_09102024/hbg",bnum,"_full_maf01to05.tsv"),header=F)
  filt <- filter(an,rsids %in% snps$V1 & rsids!="")
  filtan <- filt %>% mutate(V1=paste0("chr",`#chrom`,"_",pos,"_",ref,"_",alt)) %>% select(V1)
  data.table::fwrite(filtan, paste0("/home/ivm/from_satu/hbg_full_sets/hbg",bnum,"_maf01to05.txt"), row.names = F, col.names=F, quote = F, sep = "\t")
}

for (bnum in blocks) {
  snps <- data.table::fread(paste0("/finngen/green/hbg_09102024/hbg",bnum,"_full_maf05.tsv"),header=F)
  filt <- filter(an,rsids %in% snps$V1 & rsids!="")
  filtan <- filt %>% mutate(V1=paste0("chr",`#chrom`,"_",pos,"_",ref,"_",alt)) %>% select(V1)
  data.table::fwrite(filtan, paste0("/home/ivm/from_satu/hbg_full_sets/hbg",bnum,"_maf05.txt"), row.names = F, col.names=F, quote = F, sep = "\t")
}
...
for (nsnp in nsnps){
  for (bnum in blocks){
    hla_dir="/home/ivm/haplo_analysis/check5/"
    nsnp_dir=paste0(hla_dir,"snps",nsnp,"/")
    setwd(nsnp_dir)

    # Load in snps in the block, randomly subset
    hb_all<-fread(paste0("/home/ivm/from_satu/hbg_full_sets/hbg",bnum,"_maf05.txt"), header = F) # all snps in the block w/ MAF > 5%
    hb_all_snp <- hb_all[grep("^chr6_\\d+_[A-Z]_[A-Z]$", hb_all$V1), , drop = FALSE] # make sure just single nucleotide to single nucleotide
    set.seed(267)
    hb_half1<-hb_all_snp %>% sample_n(nsnp/2)

    #second half
    hb_all<-fread(paste0("/home/ivm/from_satu/hbg_full_sets/hbg",bnum,"_maf01to05.txt"), header = F) # all snps in the block w/ MAF > 5%
    hb_all_snp <- hb_all[grep("^chr6_\\d+_[A-Z]_[A-Z]$", hb_all$V1), , drop = FALSE] # make sure just single nucleotide to single nucleotide
    set.seed(267)
    hb_half2<-hb_all_snp %>% sample_n(nsnp/2)

    #combine
    hb <- bind_rows(hb_half1,hb_half2)
...
####### Create the genotype file for the snps in this block with the alleles (A,T,C,Gs)
##extract the snps with PLINK
##cd /home/ivm/haplo_analysis/check5/snps1000
##plink --bfile /finngen/library-red/finngen_R10/genotype_plink_1.0/data/finngen_R10 --keep-allele-order --keep /home/ivm/from_satu/general_files/individuals2.txt --recode --extract hb3_snps.txt --out hb3_genotypes
##plink --bfile /finngen/library-red/finngen_R10/genotype_plink_1.0/data/finngen_R10 --keep-allele-order --keep /home/ivm/from_satu/general_files/individuals2.txt --recode --extract hb2_snps.txt --out hb2_genotypes
##plink --bfile /finngen/library-red/finngen_R10/genotype_plink_1.0/data/finngen_R10 --keep-allele-order --keep /home/ivm/from_satu/general_files/individuals2.txt --recode --extract hb1_snps.txt --out hb1_genotypes
...
######################## hapclusterandregress_check6to14.R
## Written by Courtney Smith 9-9-2024
## Goal of script: Defining clusters and running haplotype regression pipeline - Blocks 1-3 - 30,100,150 SNPs
# Repeat this with sets of randomly selected 1000 snps, 9 times
...
#for i in {6..14}
#do
#dir_base="/home/ivm/haplo_analysis/check${i}"
#mkdir $dir_base
#dir_fig="${dir_base}/figures"
#mkdir $dir_fig
#dir="${dir_base}/snps1000"
#mkdir $dir
#done

############ Select subset of n random snps from the full set of snps in the block
list_of_seeds=c(234, 53,982,198,871,912,68,405,729) # randomly generated from between 1 to 1000
i=5 # number to generate for directory

for (myseed in list_of_seeds){
  i=i+1
  for (nsnp in nsnps){
    for (bnum in blocks){
      hla_dir=paste0("/home/ivm/haplo_analysis/check",i,"/")
      nsnp_dir=paste0(hla_dir,"snps",nsnp,"/")
      setwd(nsnp_dir)
...
for (i_version in i_list){
  for (nsnp in nsnps){
    for (bnum in blocks){
      hla_dir=paste0("/home/ivm/haplo_analysis/check",i_version,"/")
      nsnp_dir=paste0(hla_dir,"snps",nsnp,"/")
      setwd(nsnp_dir)

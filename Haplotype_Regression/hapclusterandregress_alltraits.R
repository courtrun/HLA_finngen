...
for (nsnp in nsnps){
  for (bnum in blocks){
    ...
    data.table::fwrite(cdoses, paste0(nsnp_dir,"hb",bnum,"_clusterstats_alltraits.txt"), row.names = F, quote = F, sep = "\t")
    data.table::fwrite(l, paste0(nsnp_dir,"hb",bnum,"_haplostats_alltraits.txt"), row.names = F, quote = F, sep = "\t")
    ...
    data.table::fwrite(dat_bin %>% select(-leaveout), paste0(nsnp_dir,"hb",bnum,"_preregressiondf_alltraits.txt"), row.names = F, quote = F, sep = "\t") # save df before running regression
  ...
  ##regression analysis, remember to exclude the most common cluster!
    ...
    data.table::fwrite(results_dt, paste0(nsnp_dir,"hb",bnum,"_regresults_alltraits.txt"), row.names = F, quote = F, sep = "\t")
...
sigt <- unique((results_dt %>% mutate(Z=beta/se) %>% filter(abs(Z)>=4))$nimi)

h1 <- data.table::fread(paste0("hb",bnum,"_regresults.txt")) %>%
  rename(trait=nimi) %>% mutate(Z=beta/se)
h1traits <- unique(filter(h1,abs(Z)>=4)$trait)

#compare
setdiff(sigt,h1traits) # traits sig
setdiff(h1traits,sigt)
